...
std::unordered_set<Frame*, std::hash<Frame*>, std::equal_to<Frame*>, Eigen::aligned_allocator< Frame* > > TrackableKeyFrameSearch::findCandidates(Frame* keyframe, Frame* &fabMapResult_out, bool includeFABMAP, bool closenessTH)
{
	...
	fabMapResult_out = findDeeplcdCandidate(keyframe);
	...
	std::cout<<"deeplcdResult_out is"<<fabMapResult_out<<"\n";
	...
	if (enablePrintDebugInfo && printConstraintSearchInfo)
		printf("Early LoopClosure-Candidates for %d: %d euclidean, %d appearance-based, %d total\n",
				(int)keyframe->id(), (int)fabMapResult_out->id(), appearanceBased, (int)results.size());

	return results;
}


Frame* TrackableKeyFrameSearch::findDeeplcdCandidate(Frame* keyframe)
{
	for(unsigned int i=0;i<graph->keyframesAll.size();i++){
	  mvDatabase.insert({graph->keyframesAll[i]->id(), graph->keyframesAll[i]});
	}
	
	//use DeepLCD for loop detection
	float similarityThres1 = 0.87;
	float similarityThres2 = 0.92;
	// calculate the whole image's descriptor vector with deeplcd
	keyframe->mpDescrVector = lcd->calcDescrOriginalImg(keyframe);
	...
	for(auto &db:  mvDatabase){
	  
	  // avoid comparing with recent KFs
	  if(keyframe->id() - db.first < 20) break;
	  
	  db.second->mpDescrVector = lcd->calcDescrOriginalImg(db.second);
	  float similarityScore = lcd->score(keyframe->mpDescrVector, db.second->mpDescrVector);
	  if(similarityScore > maxScore){ // record the KF candidate with the max similarity score
            maxScore = similarityScore;
            bestId = db.first;
	  }
	  
	  //if(similarityScore > similarityThres2){
          //  cntSuspected++;
	  //}
	  
	  //printf("similarityScore is %f",similarityScore);
	  
	  std::cout<<"similarityScore is"<<similarityScore<<"\n";
	  std::cout<<"maxsorce is"<<maxScore<<"\n";
	}
	...
	if(maxScore < similarityThres1){
	  return nullptr;
	}
	
	//printf("BestID is %d",bestId);
	std::cout<<"BestID is"<<bestId<<"\n";

	return mvDatabase.at(bestId);
}
