...
#include "GlobalMapping/deeplcd.h"
...
private:
	...
	Frame* findDeeplcdCandidate(Frame* keyframe);
	...
	std::string cafferoot = "/home/sadocument/caffe/examples";
	std::string model_dir = "/calc_model/";
	std::string net_def = cafferoot + model_dir + "deploy.prototxt";
	std::string net_weights = cafferoot + model_dir + "calc.caffemodel";
	lsd_slam::DeepLCD* lcd = new lsd_slam::DeepLCD(net_def, net_weights, -1);
	
	std::unordered_map<int, Frame*> mvDatabase;
