...
DeepLCD::DeepLCD(const std::string& network_definition_file, const std::string& pre_trained_model_file, int gpu_id)
{
	std::string mode = "CPU";
	if(gpu_id >= 0){
...
// ----------------------------------------------------------------------------------------
DeepLCD::DescrVector DeepLCD::calcDescrOriginalImg(Frame* originalImg)
{
	cv::Mat frame;
	cv::Mat keyFrameImage(originalImg->height(), originalImg->width(), CV_32F, const_cast<float*>(originalImg->image()));
	...
	cv::Size sz(160, 120);
	cv::Mat resizeImg;
	cv::resize(frame, resizeImg, sz);
	return calcDescr(resizeImg);
}
// ----------------------------------------------------------------------------------------
